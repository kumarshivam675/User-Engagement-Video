...
print videos_access
videos_duration = {}
i# videos_duration = get_video_duration(videos_access)
print videos_duration

def get_logs(videos, videos_duration):
	with open( path + file1, 'r') as data_file:
                for line in data_file.readlines():
                        data = json.loads(line)
                        if data['Video_Id'] == videos[0] and data['video_log'] == 'true':
                                print data['user_action']

        
	
get_logs(videos, video_duration)
'''
